...
**在线体验：[BIM 建筑平面图识别与能耗计算平台](http://43.159.58.82/energy)**

BIM Web 是一个基于 Flask 的建筑平面图 AI 识别、建筑参数配置、设计冷热负荷
和年度能耗估算平台。它把多页 PDF 图纸、ONNX 语义分割、矢量 PDF 辅助处理、
房间拓扑闭合、围护结构材料库和建筑能耗计算串成一个网页工作流。

可上传 PDF 或图片平面图，体验图纸识别、外轮廓与面积提取、比例尺标定、材料
参数配置和能耗计算流程。

## 文件格式与模型选择

本平台支持 PNG、JPG/JPEG 和 PDF 格式的建筑平面图：

- 上传 PNG、JPG/JPEG 图片时，选择“新图片 ONNX 模型”；模型会识别建筑覆盖面、
  外墙、外窗和外门，完成比例尺标定后可估算建筑面积，并按图纸主要朝向将外墙、
  外窗和外门面积填入精确计算的东、南、西、北方位。
- 上传 PDF 时使用矢量 PDF 模型。该流程适合从 CAD 等软件直接导出的、仍保留矢量
  线条和文字信息的 PDF，可利用原生线条和尺寸文字辅助识别与比例尺标定。
- 扫描版 PDF 不具备可靠的矢量信息，建议将目标楼层截图为 PNG 或 JPG 后使用图片
  模型。截图时应尽量只保留建筑主体，减少外围尺寸线、轴网编号和其他标注线，避免
  它们干扰墙体与建筑外轮廓识别。

> 演示环境仅供功能体验。请勿上传包含个人信息、商业机密或未经授权传播的图纸；
> 识别与计算结果须由具备相应资质的专业人员复核。
...
| 服务管理 | systemd |
| 代码发布 | 本地 `main` → GitHub `origin/main` → 腾讯云 |
...
页面，矢量提取最多运行 30 秒；到达时限后，系统会自动改用 100 DPI 栅格渲染
...
- 公开 GitHub 仓库不得包含密钥、账号、生产数据或内部运维记录；
