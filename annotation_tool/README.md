...
6. 保存草稿并刷新确认，再点击“确认当前标注”。
7. 只让需要训练的整页或裁剪区域处于 `confirmed` 状态，然后点击“导出已确认样本”。
...
15 秒，工具会自动以 100 DPI 栅格渲染完成准备。事件日志会显示“复杂 PDF 已
使用栅格模式准备，可正常标注墙、窗、门。”；内部预处理元数据的
`vector_analysis.mode` 会记录为 `raster_fallback`。该状态是有效的准备结果，
仍可进行墙、窗、门的标注并用于训练；不需要重新导入或重复准备页面。
